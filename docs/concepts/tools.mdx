...
<Note>
  **新功能说明 | New Feature**: AG-UI 现在支持多种工具定义模式，包括前端定义、后端定义和服务端作为客户端的模式。查看 [工具定义模式指南](/concepts/tool-definition-patterns) 了解详细信息。
  
  AG-UI now supports multiple tool definition patterns including frontend-defined, backend-defined, and server-as-client patterns. See the [Tool Definition Patterns Guide](/concepts/tool-definition-patterns) for details.
</Note>
...
AG-UI supports multiple patterns for tool definition:

1. **Frontend-defined tools**: Tools defined in frontend applications and passed to backend agents (standard pattern)
2. **Backend-defined tools**: Tools defined and executed within backend agents
3. **Server-as-client**: Backend services acting as clients to provide tools to other agents

For a comprehensive guide on these patterns, see [Tool Definition Patterns](/concepts/tool-definition-patterns).

By defining tools in the frontend and passing them to agents,
